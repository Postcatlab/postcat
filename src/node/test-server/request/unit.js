  ...
  privateFun.parseTestData = function (inputTestData, env) {
    return new Promise(async resolve => {
      ...
      try {
          ...
          case '1': {
            if (inputTestData.auth.basicAuth) {
              tmpHeaderObj['Authorization'] =
                'Basic ' +
                Buffer.from((inputTestData.auth.basicAuth.username || '') + ':' + (inputTestData.auth.basicAuth.password || '')).toString(
                  'base64'
                );
            }
            break;
        ...
        let tmpNewUri = inputTestData.URL.indexOf('?') === -1 ? '' : inputTestData.URL.replace(/(((?!\?).)*)\?/, '') || '';
        switch (tmpRequestType) {
          ...
          case '3': {
            template.formToJsonBody = _LibsCommon.bodyQueryToJson(inputTestData.params, {
              apiRequestParamJsonType: inputTestData.requestType.toString() == '2' ? inputTestData.apiRequestParamJsonType || '0' : '0',
          ...
          case '4': {
            ...
            template.body = Buffer.from((inputTestData.params.dataUrl || '').replace(/^data:(.*);base64,/gi, ''), 'base64');
        ...
        if (tmpRequestType !== '4') {
          if (!template.label.muti && /"content-type":"multipart\/form-data/i.test(JSON.stringify(template.options.headers))) {
            template.label.muti = true;
          }
        ...
        try {
          ...
          let tmpFunOprAjaxErr = (tmpInputErr, tmpInputTotalTime) => {
            ...
            tmpFunReduceAjaxResEnd = (inputRes, tmpInputResponseObj) => {
              ...
              let tmpDetected = {},
                ...
                blobFileName;
              try {
                blobFileName = _ContentDisposition.parse(inputRes.headers['content-disposition'] || 'test_response').parameters.filename;
              ...
              if (!inputRes.headers['content-type']) {
                tmpDetected =
                  _GetFileClass.byContent(
                    Buffer.concat(tmpInputResponseObj.chunk.array, tmpInputResponseObj.chunk.length) || tmpReportData.response.body
                  ) || {};
              }
              if (
                !blobFileName &&
                ...
                if (tmpReportData.response.responseLength >= 300 * 1024) {
                  tmpFileBinary = Buffer.concat(tmpInputResponseObj.chunk.array, tmpInputResponseObj.chunk.length).toString('base64');
              ...
              _LibsApiUtil
                .responsePreReduceByPromise(tmpReportData.response.body, inputTestData.afterInject, inputTestData.env || {}, {
                  ...
                  if (val.length > 0) {
                    for (let childKey in val) {
                      ...
                      switch (typeof childVal) {
                        case 'object': {
                          tmpMultiForm.append(key, Buffer.from((childVal.dataUrl || '').replace(/^data:(.*);base64,/gi, ''), 'base64'), {
          ...
          _HttpPackageClass.socketReduce(
            ...
            (tmpInputAjaxStatus, tmpInputDataObj, tmpInputXhr) => {
              ...
              let tmpHttpTotalTime = _LibsApiUtil.getMicrosToMs(tmpInputDataObj.summaryObj.startAt, tmpInputDataObj.summaryObj.endAt);
                    ...
                    if (tmpRedirectObject) {
                      ...
                      _HttpPackageClass.socketReduce(
                        ...
                        (tmpInputRedirectAjaxStatus, tmpInputRedirectDataObj) => {
                          tmpReportData.general.timingSummary.push(_LibsApiUtil.getHttpTiming(tmpInputRedirectDataObj.summaryObj));
                          ...
                          switch (tmpInputRedirectAjaxStatus) {
                            ...
                            case 'ajaxErr': {
                              tmpFunOprAjaxErr(tmpInputRedirectDataObj.errObj, tmpHttpTotalTime + tmpRedirectHttpTotalTime);
                              break;
                            }
                          }
                        }
                    ...
                    if (res.status != 'abort') {
                      ...
                      if (tmpIsOversized) {
                        ...
                        template.ajax.request.requestType =
                          tmpDecorateObj.requestType === '4' ? '1' : typeof tmpDecorateObj.requestBody.body == 'object' ? '0' : '1';
                      }
                    }
                  }
                );
