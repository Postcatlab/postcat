...
export class HttpService {
  ...
  api_envLoadByProjectId({ projectID }) {
    if (!projectID) {
      console.log('%c Error: env - loadByProjectID 接口 缺失参数 projectID %c', ErrorStyle, '');
      return;
    }
    ...
    return new Promise((resolve) => {
      this.http
        ...
        .subscribe({
          next: ({ statusCode, ...data }: any) => {
            console.log('%c env - loadByProjectID 接口请求成功 %c', SuccessStyle, '');
            if (statusCode === 200) {
              return resolve([data, null]);
            }
            resolve([null, data]);
          ...
          error: (error) => {
            console.log('%c env - loadByProjectID 接口请求失败 %c', ErrorStyle, '');
            resolve([null, error]);
          },
        });
    });
  ...
  api_apiDelete({ uuids }) {
    if (!uuids) {
      console.log('%c Error: api - delete 接口 缺失参数 uuids %c', ErrorStyle, '');
      return;
    }
  ...
  api_testDelete({ uuids }) {
    if (!uuids) {
      console.log('%c Error: test - delete 接口 缺失参数 uuids %c', ErrorStyle, '');
      return;
    }
