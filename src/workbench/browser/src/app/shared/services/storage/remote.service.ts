...
export class RemoteService {
  ...
  api_workspaceEdit({ workspanID }) {
    if (!workspanID) {
      console.log('%c Error: workspace - edit 接口 缺失参数 workspanID %c', ErrorStyle, '');
      return;
    }
  ...
  api_workspaceDelete({ workspanID }) {
    if (!workspanID) {
      console.log('%c Error: workspace - delete 接口 缺失参数 workspanID %c', ErrorStyle, '');
      return;
    }
  ...
  api_workspaceMember({ workspanID }) {
    if (!workspanID) {
      console.log('%c Error: workspace - member 接口 缺失参数 workspanID %c', ErrorStyle, '');
      return;
    }
  ...
  api_workspaceAddMember({ workspanID }) {
    if (!workspanID) {
      console.log('%c Error: workspace - addMember 接口 缺失参数 workspanID %c', ErrorStyle, '');
      return;
    }
  ...
  api_workspaceRemoveMember({ workspanID }) {
    if (!workspanID) {
      console.log('%c Error: workspace - removeMember 接口 缺失参数 workspanID %c', ErrorStyle, '');
      return;
    }

    return new Promise((resolve) => {
      this.http.get(`/api/workspace/${workspanID}/member/remove`, {}).subscribe({
        next: ({ statusCode, ...data }: any) => {
  ...
  api_userReadProfile(params) {
    return new Promise((resolve) => {
      this.http.get(`/api/user/profile`, { headers: {}, params }).subscribe({
        next: ({ statusCode, ...data }: any) => {
  ...
  api_userSearch({ usernme }) {
    if (!usernme) {
      console.log('%c Error: user - search 接口 缺失参数 usernme %c', ErrorStyle, '');
      return;
    }
  ...
  api_authLogout({ refreshTokenExpiresAt }) {
    if (!refreshTokenExpiresAt) {
      console.log('%c Error: auth - logout 接口 缺失参数 refreshTokenExpiresAt %c', ErrorStyle, '');
      return;
    }
