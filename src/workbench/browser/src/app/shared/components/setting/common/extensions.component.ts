    ...
    <form nz-form [nzLayout]="'vertical'" [formGroup]="validateForm" class="form">
      <div *ngFor="let module of extensitonConfigurations">
        ...
        <nz-form-item nz-col class="fg1" *ngFor="let field of objectKeys(module.properties || {})">
          <ng-container *ngIf="module.properties[field]?.title||module.properties[field]?.label">
            <nz-form-label nzFor="{{ field }}" [nzRequired]="module.properties[field]?.required" class="label">
              {{ module.properties[field]?.title||module.properties[field]?.label }}
            </nz-form-label>
          </ng-container>
          <!-- 二级说明 -->
          <div
          ...
          <nz-form-control
            i18n-nzErrorTip
            nzErrorTip="Please Enter {{ module.properties[field]?.title||module.properties[field]?.label }}"
            class="form-control"
          >
            <!-- 字符串类型 -->
