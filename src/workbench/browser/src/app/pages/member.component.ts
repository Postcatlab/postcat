  ...
  template: `<nz-modal
    ...
    >
      <ng-container *nzModalContent>
        ...
        <button
          ...
          nzType="primary"
          nzBlock
          (click)="btn497ppyCallback()"
          [disabled]="btn9pzlshStatus()"
          i18n
        >
    ...
    <section class="py-5 px-10">
      <h2 class="text-lg flex justify-between items-center">
        ...
        ><button nz-button class="" nzType="primary" (click)="btnev7tagCallback()" i18n>Add people</button>
      </h2>
      <section class="py-5">
        <eo-manage-access [data]="memberList" (eoOnRemove)="e8zb5reCallback($event)"></eo-manage-access>
      </section>
    </section>`,
})
export class MemberComponent implements OnInit {
  ...
  async e68kbf9Callback() {
    // * nzAfterClose event callback
    this.inputPersonValue = '';
  }
  async btn497ppyCallback() {
  ...
  async btnev7tagCallback() {
    // * click event callback

    // * 唤起弹窗
    this.isInvateModalVisible = true;
  }
  async e8zb5reCallback($event) {
