  ...
  template: ` <nz-modal
    ...
    >
      ...
      <ng-template #modalSyncFooter>
        <button nz-button class="" nzType="default" (click)="btn7zp4esCallback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btnzdyw4gCallback()" i18n>Sync</button>
      </ng-template>
    </nz-modal>
    <nz-modal
    ...
    >
      ...
      <ng-template #modalCheckConnectFooter>
        <button nz-button class="" nzType="default" (click)="btnz6qp8bCallback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btnl7hvh7Callback()" i18n>Retry</button>
      </ng-template>
    </nz-modal>
    <nz-modal
      [nzFooter]="null"
    ...
    >
      <ng-container *nzModalContent>
        <section class="my-3">
          ...
          <button nz-button class="h-10" nzType="primary" nzBlock (click)="btnqxm6chCallback()" i18n>Sign In/Up</button>
        </section>
      </ng-container>
    </nz-modal>
    <nz-modal
    ...
    >
      <ng-container *nzModalContent>
        ...
        <span style="color: #1890ff" (click)="textjtv7frCallback()" i18n> open the settings </span>
    ...
    >
      ...
      <ng-template #modalAddWorkspaceFooter>
        <button nz-button class="" nzType="default" (click)="btnkqnnb1Callback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btndhs7jbCallback()" i18n>Save</button>
      </ng-template>
    </nz-modal>`,
})
export class UserModalComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.project.getWorkspaceInfo(workspaceID);
  ...
  async btn7zp4esCallback() {
    // * click event callback

    // * 关闭弹窗
    this.isSyncModalVisible = false;
  }
  async btnzdyw4gCallback() {
    // * click event callback
  ...
  async btnz6qp8bCallback() {
    // * click event callback

    // * 关闭弹窗
    this.isCheckConnectModalVisible = false;
  }
  async btnl7hvh7Callback() {
    // * click event callback
  ...
  async eduo8uzCallback() {
    // * nzAfterClose event callback

    // * Clear Username form
    this.validateUsernameForm.reset();
  }
  async btnqxm6chCallback() {
    // * click event callback
  ...
  async btnkqnnb1Callback() {
    // * click event callback

    // * 关闭弹窗
    this.isAddWorkspaceModalVisible = false;
  }
  async btndhs7jbCallback() {
    // * click event callback
