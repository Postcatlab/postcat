  ...
  template: ` <nz-modal
    ...
    >
      ...
      <ng-template #modalSyncFooter>
        <button nz-button class="" nzType="default" (click)="btn9e98qlCallback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btnilbe47Callback()" i18n>Sync</button>
      </ng-template>
    </nz-modal>
    <nz-modal
    ...
    >
      ...
      <ng-template #modalCheckConnectFooter>
        <button nz-button class="" nzType="default" (click)="btny2w9goCallback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btnile2vpCallback()" i18n>Retry</button>
      </ng-template>
    </nz-modal>
    <nz-modal
      [nzFooter]="null"
    ...
    >
      <ng-container *nzModalContent>
        <section class="my-3">
          ...
          <button nz-button class="h-10" nzType="primary" nzBlock (click)="btnffp3asCallback()" i18n>Sign In/Up</button>
        </section>
      </ng-container>
    </nz-modal>
    <nz-modal
    ...
    >
      <ng-container *nzModalContent>
        ...
        <span style="color: #1890ff" (click)="texttquoagCallback()" i18n> open the settings </span>
    ...
    >
      ...
      <ng-template #modalAddWorkspaceFooter>
        <button nz-button class="" nzType="default" (click)="btnd6ji6pCallback()" i18n>Cancel</button>
        <button nz-button class="" nzType="primary" (click)="btnov0wicCallback()" i18n>Save</button>
      </ng-template>
    </nz-modal>`,
})
export class UserModalComponent implements OnInit {
  ...
  async btn9e98qlCallback() {
    // * click event callback

    // * 关闭弹窗
    this.isSyncModalVisible = false;
  }
  async btnilbe47Callback() {
    // * click event callback
  ...
  async btny2w9goCallback() {
    // * click event callback

    // * 关闭弹窗
    this.isCheckConnectModalVisible = false;
  }
  async btnile2vpCallback() {
    // * click event callback
  ...
  async ejw0nlqCallback() {
    // * nzAfterClose event callback

    // * Clear Username form
    this.validateUsernameForm.reset();
  }
  async btnffp3asCallback() {
    // * click event callback
  ...
  async btnd6ji6pCallback() {
    // * click event callback

    // * 关闭弹窗
    this.isAddWorkspaceModalVisible = false;
  }
  async btnov0wicCallback() {
    // * click event callback
